...
class CustOps(callbacks.Plugin):
    ...
    def social(self, irc, msg, args, channel, user, junk):
        ...
        Sets a redirection ban from #powder to #powder-social, kicks the user (exploiting a users auto-rejoin to force them to #powder-social) then lifts the ban.
...
#       if msg.nick.lower() is user.lower(): irc.error('Nice try')
    ...
    def selfInfo(self, irc, msg, args, line):
        ...
        try:
            i = self.infoLines[msg.nick.lower()]
        except:
            self._getInfo()

        try:
            self.infoLines[msg.nick.lower()]
        except:
            self.infoLines[msg.nick.lower()] = {}
